...
### Changed
- Bump bundled `mgt` platform binaries from 0.4.0 to 0.5.0

## [0.11.4] - 2026-09-23

### Fixed
- Fix npm audit vulnerabilities in dev dependencies (vitest, esbuild, vite, postcss, nanoid)

## [0.11.3] - 2026-09-23

### Changed
- Bump bundled `mgt` platform binaries from 0.3.8 to 0.4.0
...
### Changed
- Update skill and README prerequisites to reflect that the `mgt` binary is now bundled automatically
...
- Handle already-running containers in `start_source` — returns success with "already running" message instead of throwing a container name conflict error
- Unit tests for `start-source.ts` covering success, already-running, rollback, and missing-port scenarios

### Changed
- Improve timeout error messages with actionable guidance (check Docker, verify network, retry)

### Removed
- Remove unused `"stopped"` status from `list_sources` response type — `mgt ps` cannot distinguish stopped from never-started
...
- Add top-level `description`, `owner.email`, plugin `category`, and `tags` to `marketplace.json`; change owner to personal name
...
[Unreleased]: https://github.com/model-graph-tools/claude-plugin/compare/v0.11.4...HEAD
